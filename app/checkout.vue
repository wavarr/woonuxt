...
const payNow = async () => {
  ...
  try {
    if (orderInput.value.paymentMethod.id === 'btcpay') {
      ...
      try {
        // Fetch from API endpoint directly instead of using GraphQL
        const response = await $fetch('/api/btcpay/intent', {
          method: 'POST',
          body: { 
            amount: cart.value.total, 
            currency: runtimeConfig.public.currencyCode 
          }
        });
        
        // Extract the btcPaymentIntent from the response
        ...
        orderInput.value.transactionId = btcPaymentIntent?.transactionId || new Date().getTime().toString();
      } catch (error) {
        console.error('BTCPay intent error:', error);
        // Fallback to timestamp transaction ID if there's an error

...
const checkPaymentStatus = async () => {
  try {
    const response = await fetch(
      `${process.env.GQL_HOST?.replace('graphql', '') || '/api'}?wc-api=BTCPay_Check_Payment&order_id=${orderId.value}&order_key=${orderKey.value}`
    );
...
const initializeCheckout = async () => {
  try {
    const response = await fetch(
      `${process.env.GQL_HOST?.replace('graphql', '') || '/api'}?wc-api=BTCPay_Checkout&order_id=${orderId.value}&key=${orderKey.value}`
    );
